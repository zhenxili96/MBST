...
class VID:
  def __init__(self, imdb_path, max_frame_dist, with_cluster, epoch_size=None):
    ...
    if with_cluster:
        self.video_cluster = imdb['video_cluster']
    else:
        self.video_cluster = None
  ...
  def __getitem__(self, index):
    ...
    if self.video_cluster is None:
        ...
        for j, frame_idx in enumerate(out_idxs):
          img_path = img_ids[frame_idx]
          video.append(img_path.encode('utf-8'))
        cur_cluster = self.video_cluster[index % len(self.videos)]
        video.append(str(cur_cluster))
        return video
